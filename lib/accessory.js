  ...
  for (var k in this.services) {
    for (var i in this.services[k].characteristics) {
      if (this.services[k].characteristics[i].displayName !== "Name") {
        ...
        this.log.debug("Accessory.getService_names '%s' %s '%s' '%s'", this.name, this.services[k].characteristics[i].iid, this.services[k].characteristics[i].displayName, this.services[k].displayName);
      }
    }
  }
  this.log.debug("");
...
Accessory.prototype.get = function(callback, context, displayName, iid) {
  ...
  if (this.reachable) {
    ...
    //this.log.debug("Accessory.get %s", JSON.stringify(this.services, null , 2));
    ...
    this.log.debug("Accessory.get %s %s %s", iid, service_name, c);
