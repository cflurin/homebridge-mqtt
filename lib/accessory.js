...
Accessory.prototype.addService = function(accessory, service_name, subtype) {
  ...
  if (typeof Service[service_name] !== "undefined") {
    if (typeof subtype === "undefined") { // primary service
      ...
      //this.log.debug("Accessory.addService %s", JSON.stringify(this.subtypesList));
...
Accessory.prototype.addSubtype = function(subtype) {
  ...
  this.subtypesList.push(subtype);
}
...
Accessory.prototype.getSubtype = function(accessory) {
  ...
  for (var k in accessory.services) {
    for (var i in accessory.services[k].characteristics) {
      if (accessory.services[k].characteristics[i].displayName !== "Name") {
        this.subtypes[accessory.services[k].characteristics[i].iid] = accessory.services[k].subtype;
      }
    }
  }
  //this.log.debug("Accessory.getSubtype %s", JSON.stringify(this.subtypes, null, 2));
}
