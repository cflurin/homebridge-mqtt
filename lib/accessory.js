...
Accessory.prototype.addService = function(accessory, service_name, service_type) {
  ...
  if (typeof Service[service_type] !== "undefined") {
    ...
    accessory
      ...
      .setCharacteristic(Characteristic.Model, service_type)
...
Accessory.prototype.configureAccessory = function(accessory, service_name, service_type) {

  //this.log.debug("Accessory.configureAccessory %s %s %s", this.name, service_name, service_type);
    
  if (typeof service_type !== "undefined") {  // new accessory
    ...
    this.configureCharacteristics(accessory, service_name);
    ...
    for (var k in accessory.services) {
      if (typeof accessory.services[k].displayName !== "undefined") {
        ...
        this.log.debug("Accessory.configureAccessory displayName %s", this.services[service_name].displayName);
        this.configureCharacteristics(accessory, service_name);
      }
    }
...
Accessory.prototype.configureCharacteristics = function(accessory, service_name) {
  ...
  for (var k in service.characteristics) {
    ...
    //this.log.debug("Accessory.configureCharacteristics %s %s %s", this.name, service_type, c);
    ...
    if (c != "Name") {
      ...
      //this.log.debug("Accessory.configureCharacteristics %s %s %s %s", this.name, service_type, c, JSON.stringify(this.i_props));
    }
  }
  //this.log.debug("Accessory.configureCharacteristics Def %s", JSON.stringify(this.accessoryDef[c]));
  ...
  for (var k in service.optionalCharacteristics) {
    ...
    if (typeof(this.accessoryDef[c]) !== "undefined") {
      this.log.debug("Accessory.configureCharacteristics %s %s optional %s", this.name, service_type, c);
