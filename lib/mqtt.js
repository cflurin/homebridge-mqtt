...
Mqtt.prototype.connect = function() {
  ...
  if(options.username === null || options.password === null) {
    this.log("Mqtt.connect Anonymous");
    client = mqtt.connect(url);
  } else {
    this.log("Mqtt.connect Authentication with username/password");
  ...
  client.on('connect', function () {
    ...
    this.log("Mqtt connected (url = %s)", url);
    if (this.config.username) this.log.debug("Mqtt.on.connect %s %s", this.config.username, this.config.password);
  ...
  client.on('message', function (topic, buffer) {
    ...
    if (typeof topic === "undefined" || payload.length === 0) {
      ...
      this.log.debug("Mqtt.on.message %s", message);
      this.sendAck(false, message);
    } else {
    
      //this.log.debug("Mqtt.on.message topic %s payload %s", topic, payload);
      ...
      try {
        ...
        this.log.debug("Mqtt.on.message %s (%s)", message, e.message);
        this.sendAck(false, message);
        isValid = false;
      }
        ...
        switch (topic) {
          ...
          case topic_prefix + "/to/set/reachable":
            if (typeof accessory.reachable === "boolean") {
              ...
              this.log.warn("Mqtt.on.message %s", message);
              this.sendAck(false, message);
            }
            break;
          ...
          default:
            ...
            this.log.warn("Mqtt.on.message default %s", message);
            this.sendAck(false, message);
        }
      }
    }
  ...
  client.on('error', function (error) {
    this.log.error("Mqtt.on.error %s", error);
  }.bind(this));
  
  client.on('reconnect', function () {
    this.log.debug("  <to analyze> Mqtt.on.reconnect");
  }.bind(this));
  
  client.on('close', function () {
    this.log.debug("  <to analyze> Mqtt.on.close");
  }.bind(this));

  client.on('offline', function () {
    this.log.debug("  <to analyze> Mqtt.on.offline");
  }.bind(this));
