...
Mqtt.prototype.connect = function(url) {
  ...
  client.on('message', function (topic, buffer) {
    ...
    this.log.debug("on message %s", topic);
    ...
    switch (topic) {
      case "homebridge/to/add":
        this.log.debug("on messsage add %s", JSON.stringify(accessory, null, 2));
      ...
      case "homebridge/to/get":
        var name;
        this.log.debug("on message get %s", JSON.stringify(accessory));
...
var msg = accessories;
this.log.debug("sendAccessories %s", JSON.stringify(msg, null, 2));
