...
var plugin_name, topic_prefix, accessories, Characteristic, platform_name;
...
Mqtt.prototype.on_set = function(accessory) {
  ...
  if (result.isValid) {
    result = accessories[accessory.name].save_and_setValue(platform_name, result.service_name, accessory.characteristic, result.value);
    if (!result.isValid) {
