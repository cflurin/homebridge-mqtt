...
Mqtt.prototype.on_set = function(accessory) {
  ...
  if (typeof accessory.service_name === "undefined") {
    accessory.service_name = accessory.name;
  }
  ...
  if (result.isValid) {
    accessories[accessory.name].save_and_setValue("Mqtt", accessory.service_name, accessory.characteristic, result.value);
  } else {
...
Mqtt.prototype.validate = function(accessory) {
  ...
  } else if (typeof(accessories[name].services[service_name].getCharacteristic(Characteristic[c])) === "undefined"){
    message = "name '" + name + "' service_name '" + service_name + "' characteristic do not match.";
  } else {
    var result = {};
    result = accessories[name].parseValue(service_name, c, value);
...
Mqtt.prototype.identify = function (name) {
