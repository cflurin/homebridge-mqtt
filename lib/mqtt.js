...
var addAccessory, removeAccessory, addService, getAccessories, updateReachability;
...
Mqtt.prototype.on_set = function(accessory) {
  ...
  if (result.isValid) {
    accessories[accessory.name].save_and_setValue("Mqtt", accessory.subtype, accessory.characteristic, result.value);
  } else {
...
Mqtt.prototype.validate = function(accessory) {
  ...
  } else if (typeof(accessories[name].services[subtype].getCharacteristic(Characteristic[c])) === "undefined"){
    message = "name '" + name + "' subtype '" + subtype + "' characteristic do not match.";
  } else {
    var result = {};
    result = accessories[name].parseValue(subtype, c, value);
...
Mqtt.prototype.get = function(name, subtype, c, callback) {

  this.log.debug("Mqtt.get %s %s %s", name, subtype, c);
  var msg = {"name": name, "subtype": subtype, "characteristic": c};
  client.publish(topic_prefix + '/from/get', JSON.stringify(msg), this.publish_options);
  // callback(null, null);  // not used
}

Mqtt.prototype.set = function(name, subtype, c, value, callback) {
     
  this.log.debug("Mqtt.set %s %s %s %s", name, subtype, c, value);
  var msg = {"name": name, "subtype": subtype, "characteristic": c, "value": value};
