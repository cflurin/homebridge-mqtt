...
Mqtt.prototype.on_set = function(accessory) {
  ...
  if (result.isValid) {
    accessories[accessory.name].save_and_setValue("Mqtt", result.service_name, accessory.characteristic, result.value);
  } else {
...
Mqtt.prototype.validate = function(accessory) {
  ...
  if (typeof service_name === "undefined") {
    if (typeof accessories[name].hap_accessory.context.service_name === "undefined") {
      message = "name '" + name + "', service_name undefined.";
      return {isValid: false, message: message, value: value};
    } else {
      service_name = name;
    }
